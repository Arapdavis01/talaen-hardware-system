const AdminReportsComponent = {
    ...
    loadToday() {
        fetch('/api/daily-reports/today').then(function(r){return r.json();}).then(function(report){
            ...
            h += '</div>'; document.getElementById('todayReport').innerHTML = h;
        });
    },
    loadHistory() {
        var self = this;
    ...
    _displayReports(reports) {
        ...
        if(!reports.length){h+='<tr><td colspan="6">No reports found.</td></tr>';}
        ...
        h+='</tbody></table>'; document.getElementById('reportsHistory').innerHTML = h;
    },
    filterByDate() {
        ...
        var filtered = reports.filter(function(r) {
            var matchDate = true; var matchSearch = true;
    ...
    showDayDetail(reportDate) {
        fetch('/api/sales').then(function(r){return r.json();}).then(function(sales){
            ...
            var payments = {}; var cashiers = {};
            daySales.forEach(function(s) {
                var pm = s.paymentMethod || 'cash'; if (!payments[pm]) payments[pm] = { count: 0, total: 0 }; payments[pm].count++; payments[pm].total += Number(s.total || 0);
                var cn = s.cashierName || 'Unknown'; if (!cashiers[cn]) cashiers[cn] = { count: 0, total: 0 }; cashiers[cn].count++; cashiers[cn].total += Number(s.total || 0);
            });
            ...
            daySales.forEach(function(s){var d=new Date(s.date);h+='<tr><td><strong>'+(s.receiptNo||'')+'</strong></td><td>'+d.toLocaleTimeString('en-KE',{hour:'2-digit',minute:'2-digit'})+'</td><td>'+(s.customerName||'Walk-in')+'</td><td>'+(s.items?s.items.length:0)+'</td><td>KES '+Number(s.total||0).toLocaleString()+'</td><td>'+(s.paymentMethod||'cash').toUpperCase()+'</td></tr>';});
            h+='</tbody></table></div>';
