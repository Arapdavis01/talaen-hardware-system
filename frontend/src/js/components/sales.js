        ...
        .then(function(d){
            ...
            if(d.success){
                ...
                var checkStatus = setInterval(function(){
                    attempts++;
                    // ✅ REPLACED: fetch with ApiService
                    ApiService.get('/mpesa/transaction/' + d.checkoutRequestID)
                    .then(function(t){
                        if(t.status === 'completed'){
                            ...
                            setTimeout(function(){ 
                                m.remove(); 
                                if(onComplete) onComplete({mpesaRef: t.mpesaReceiptNumber, phone: phone}); 
                            }, 1000);
                        ...
                        if(attempts > 30) { 
                            clearInterval(checkStatus); 
                            statusDiv.innerHTML = '<span style="color:#f59e0b;">Payment check timeout. Please verify manually.</span>'; 
                        }
                    });
                }, 2000);
            } else {
